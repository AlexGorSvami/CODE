...
#     links = browser.find_elements(By.LINK_TEXT, '16243162441624')
#     for link in links:
#         link.click()
#         time.sleep(5)
...
#     print(sum([int(i.text) for i in links]))
...
#     links = browser.find_elements(By.CLASS_NAME, "check")
#     for link in links:
#         link.click()
...
#     links = browser.find_elements(By.CLASS_NAME, "check")
#     for link in links:
#         if int(link.get_attribute('value')) in numbers:
#             link.click()
#     button  = browser.find_element(By.CLASS_NAME, "btn")
#     button.click()
#     time.sleep(5)
...
#     elements = [int(i.text) for i in browser.find_elements(By.TAG_NAME, 'option')]
#     input = browser.find_element(By.ID, 'input_result').send_keys(sum(elements))
...
with webdriver.Chrome() as browser:
    ...
    expression = eval(browser.find_element(By.ID, 'text_box').text)
    elements = browser.find_elements(By.TAG_NAME, 'option')
    for i in elements:
        if int(i.text) == expression:
            i.click()
            break
    button = browser.find_element(By.CLASS_NAME, 'btn_box').click()
    time.sleep(5)
